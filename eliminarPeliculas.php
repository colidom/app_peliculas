...
if (($VALIDADO==1) && ($tipo_usuario==0)) //Si el usuario se ha validado y es administrador
{
  ...
  if (isset($eliminar))  // Se borran las peliculas selccionadas en el formulario
  {
	...
	if (isset($_POST['borrar']))
	{
      ...
      echo "<tr >";
           ...
           echo "<td class=\"mostrarCabecera\" align=\"center\"> G�NERO</td>";
      echo "</tr>";
      echo "<tr >";
      ...
      for ($i=0; $i<$nfilas; $i++)
      {
          ...
          // Obtener datos de la noticia i-�sima
           ...
           $result_delete = mysqli_query($con, $sql_delete)
              or die ("Fallo en la eliminaci�n de las puntuaciones de la película:.".$reg_query['nombre']);

           // Eliminar película
           $sql_delete = "delete from peliculas where codigo_pelicula=".$borrar[$i];
           $result_delete = mysqli_query($con, $sql_delete)
              or die ("Fallo en la eliminaci�n de la película:.".$reg_query['nombre']);
  ...
  else  //Se muestra el formulario
  {
        ...
        // *** Construcci�n de la consulta de busqueda ***
        ...
        //Incluido el script de paginaci�n
        $_pagi_sql=$sql;
