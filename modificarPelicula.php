...
$conexion->cerrar_conexion();
    } else // Si no se ha enviado el fornulario o hay errores al enviarlo
    {
     ...
     <tr border="0">
      ...
      <tr border="1">
          <td colspan="4" class="formularioTitulo"> MODIFICAR PELÚCULA </td>
      </tr>
  ...
  <?php
      ...
      <tr >
        ...
        ?>>Aventuras</option>
                   <option value="Terror" <?php if (strcmp($reg["genero"], 'TERROR') == 0) {
            echo " selected=\"true\"";
        }
        ?> >Terror</option>
                   <option value="Comedia" <?php if (strcmp($reg["genero"], 'COMEDIA') == 0) {
            echo " selected=\"true\"";
        }
        ?> >Comedia</option>
                   <option value="Accion" <?php if (strcmp($reg["genero"], 'ACCION') == 0) {
            echo " selected=\"true\"";
        }
        ?> >Accion</option>
                   <option value="Belica" <?php if (strcmp($reg["genero"], 'BELICA') == 0) {
            echo " selected=\"true\"";
        }
        ?> >Belica</option>
                   <option value="Drama" <?php if (strcmp($reg["genero"], 'DRAMA') == 0) {
            echo " selected=\"true\"";
        }
        ?> >Drama</option>
                   <option value="Suspense" <?php if (strcmp($reg["genero"], 'SUSPENSE') == 0) {
            echo " selected=\"true\"";
        }
        ?> >Suspense</option>
                   <option value="C. Ficcion" <?php if (strcmp($reg["genero"], 'C. FICCION') == 0) {
            echo " selected=\"true\"";
        }
        ?> >C.Ficcion</option>
               </select>
          ...
          <td class="formulario" > SI <input type="radio" name="publico" value="1" <?php if (!isset($reg["publico"]) || 1 == $reg["publico"]) {
            echo "checked";
        }
        ?> > NO <input type="radio" name="publico" value="0"  <?php if (isset($reg["publico"]) && 0 == $reg["publico"]) {
            echo "checked";
        }
        ?> ></td>
      </tr>
  ...
  <?php
if (true == $error && "" != $errores["director"]) {
            echo "<tr border=\"1\">";
      ...
      <tr >
          ...
          <td class="formulario" colspan="3"> SI <input type="radio" name="estreno" value="1" <?php if (!isset($reg["estreno"]) || 1 == $reg["estreno"]) {
            echo "checked";
        }
        ?> > NO <input type="radio" name="estreno" value="0"  <?php if (isset($reg["estreno"]) && 0 == $reg["estreno"]) {
            echo "checked";
        }
        ?> ></td>
      </tr>
